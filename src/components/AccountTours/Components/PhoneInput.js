...
const PhoneInput = ({
	...
	useEffect(() => {
		if (region_code) {
			setCurrentRegion(COUNTRIES.filter((item) => item.code === region_code)[0])
		} else if (language) {
			setCurrentRegion(
				COUNTRIES.filter((item) => item.iso === language.toUpperCase())[0]
			)
		}
			...
			<>
				<div className="region-select-modal-wrapper">
					<div ref={modal_body} className="region-select-modal-body">
						<div
							ref={modal_body}
							className="region-select-modal-body-elements-wrapper"
						>
							{COUNTRIES.map((item, index) => (
								<>
									<div
										key={index}
										className="region-select-modal-body-element"
										onClick={() => {
		...
		<>
			<div className="with-errors-wrapper" style={{ margin: margin }} id={name}>
				...
				<div className={`phone-select-input-wrapper`}>
					<div
						className={`phone-select-input ${
							currentError?.length > 0 ? 'error' : 'ok'
						}`}
					>
						...
						<div
							ref={code_ref}
							className="phone-select-input-country_code-wrapper"
						>
							{`+${regionCode}`}
						</div>
						<InputMask
				...
				<div className="errors-list">
					...
					<ul>
						{Array.isArray(currentError) &&
							...
							currentError.map((item, index) => <li key={index}>{item}</li>)}
					</ul>
				</div>
			</div>
		</>
